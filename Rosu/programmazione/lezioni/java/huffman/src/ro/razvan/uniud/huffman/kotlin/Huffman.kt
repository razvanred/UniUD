...
object Huffman {
    ...
    object Encoder {
        ...
        fun huffmanTree(freq: IntArray): Node {
            ...
            freq.filter { it > 0 }.forEachIndexed { index, element ->
                queue.add(Node(index.toChar(), element))
            }
         ...
         *
         */
        private fun compileTable(node: Node, code: String, codes: Array<String?>) {
            if (node.isLeaf) {
                codes[node.character.toInt()] = code
            } else {
                compileTable(node.left!!, "${code}0", codes)
                compileTable(node.right!!, "${code}1", codes)
            }
        }
