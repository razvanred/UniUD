...
;; Razvan Rosu
;; Tassellazione piastrelle

(define L-tessellation ; image
  (lambda (k) ; int (potenza di due)
    (if (<= k 1)
        L-tile
        (let ([nextL (L-tessellation (floor (/ k 2)))])
          (glue-tiles
           (glue-tiles
            nextL
            (shift-down (quarter-turn-left nextL) k))
           (glue-tiles 
            (shift-right (quarter-turn-left (half-turn nextL)) k)
            (shift-right (shift-down nextL (floor (/ k 2))) (floor (/ k 2)))
           )
          )
        )
    )))
