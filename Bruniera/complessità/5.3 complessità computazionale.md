...
> Una k-I/O-TM $M$ decide $L$ in spazio $f(n)$ se $\forall_x~|x|=n$, si ha che $M(x)\downarrow$ usando al massimo spazio $f(n)$.
...
Possiamo dimostrarlo trovando una macchina $M$ per simulare una macchina $N$ in tempo $c^{f(n)}$.
Possiamo ottenere "facilmente" una macchina che simuli l'esecuzione di $N$ facendo una sorta di visita DFS del grafo di computazione. Nel caso peggiore la dimensione del grafo è $d^{f(n)}$ dove $d$ è il grado di non determinismo.\
...
* Supponiamo una ND-I/O-k-MdT, per un input $x$ la complessità di spazio è il massimo spazio usato da un ramo della computazione
...
* $NPSPACE=\bigcup\limits_{k\in\N}NSPACE(n^k)$
  * $PSPACE=NPSPACE$ lo sappiamo, è facile
...
$f$ è una funzione propria, quindi anche $f'(n)=f(n)+5n+4$ è propria e può essere calcolata in unario in $f(|x|)+5|x|+4$ passi.\
