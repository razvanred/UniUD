...
La parola design è molto ambigua, anzitutto è sia sostantivo che verbo.
Come verbo significa: disegnare, ideare, creare, eccetera.

Si può avere una visione ingegneristica o artistica sul design, anche sulle interfacce utente ci sono filosofie di pensiero da entrambe le visioni.

Sono spesso molto richiesti designer "artisti" che producano le interfacce, per evitare di avere siti tutti uguali e noiosi.
Ma ovviamente non si può avere siti radicalmete diversi l'uno dall'altro, altrimenti non potremmo avere interfacce unificate.
...
Se dovessimo fare interfacce solo pe astronauti, questi discorsi sarebbero inutili, loro sono disposti ad utilizzare interfacce completamente non intuitive.
L'interfaccia con cui sono andati sulla luna prevedeva di selezionare un verbo (ad esempio "chiudere") ed un sostantivo (ad esempio "pannelli") premendo un tasto con scritto verbo o sostantivo, e poi digitando il numero che rappresenta quello che vogliamo.

## User centered design

Metodo di design che da all'utente la sensazione che l'interfaccia sia stata sviluppata per loro.
...
È scomodo da gestire ed organizzare quindi molto raro, soprattutto in Italia, ma è potenzialmente molto efficace.
...
A questo punto dobbiamo capire che input bisogna dare al programma per svolgere le varie operazioni, coinvolgendo gli utenti in dei workshop per capire il modo migliore.
...
Ad esempio, per il task "salvare un file" un utente inesperto suggerirà un tasto, un altro utente suggerirà ctrl+c.

Un altro esempio in cui non si riesce a soddisfare tutti è la programmazione di un forno.
...
Si possono usare dei programmi specifici per progettare  questi alberi di task, come CTTE o XMind

### Scenari/Casi d'uso

Si tratta di situazioni tipo in cui dobbiamo immaginare il comportamento di un utente.
Una buona idea è disegnarli come un fumetto o comunque una storyboard, come se stessimo scrivendo una sceneggiatura, solo che la storia non è lineare.
...
Il coinvolgimento dell'utente nel test dell'interfaccia attraverso scenari fa parte della prototipazione tipica della progettazione
...
Si ha molto spazio per la creatività, ma anche la possibilità di scegliere tra le interfacce statisticamente più di successo della storia, rinunciando completamente alla creatività.

Esistono delle guidelines che stabiliscono dei suggerimenti su come progettare un'interfaccia in modo che tutti i programmi siano coerenti tra di loro.
...
Una pietra miliare sono le linee guida di Apple del '92, aggiornate qualche anno dopo. Seguendo il loro esempio Microsoft (un po' in ritardo, nel '98) ha pubblicato le sue, anche se un po' striminzite.\
Interfacce di questo tipo servono per uniformare le applicazioni per la stessa piattaforma, lasciando comunque un po' di spazio per la creatività senza violare le indicazioni.
Non ha senso seguire le interfacce di un sistema facendo applicazioni per un altro, dovremmo, inveve, prima di sviluppare un programma per un sistema, controllare se hanno rilasciato delle linee guida e seguire quelle.
...
Ad esempio IBM ha sviluppato delle linee guida per le applicazioni 3D d'ufficio, in questo modo, se qualcuno vuole imbarcarsi in un progetto di sviluppo di una particolare applicazione abbiamo in indicazione di cosa funzione per applicazioni di questo tipo.

Riguardo le piattaforme recenti sono disponibili linee guida per:

* Apple Human Interface Guidelines for MacOs 2009 2020
...
Interessantemente, negli ultimi anni le linee guida di Apple lasciano un po' a desiderare mentre quelle di Microsoft sono diventate molto sofisticate
Anche se inizialmente la situazione era al contrario.

Durante la produzione della nostra interfaccia è una buona idea creare dei prototipi per testarli, darli in mano agli utenti, ed eventualmente tornare alla fase di progettazione.
...
Negli anni più recenti è stato prodotto un nuovo standard per i dispositivi multimediali, ma l'idea è la stessa.

### Protitipazione

L'idea è di ciclare più volte tra una fase di analisi, progettazione, e prototipazione.
...
Le specifiche che abbiamo prodotto a questo punto diventano le specifche finali del prodotto, salvo futuri aggiornamenti.
...
È utile avere una checklist di quello che si deve inserire nella documentazione per avere un help funzionale:

* Esplorazione
  * Cosa posso fare con questo sistema?
  * In questa sezione è utile inserire dei tutorial del sistema
...
* Efficacia: l'accuratezza e completezza con cui gli utenti possono raggiungere gli obbiettivi in ambienti particolari
...
Ampliamo il modello del model human processor, che divideva il modo di interagire in funzioni del sistema percettivo, cognitivo e motorio.
...
I due semicicli "Esecuzione" e "Valutazione" (le frecce nel modello di Norman) sono chiamate golfi, e separano quello che è in testa all'utente, da quello che viene comunicato all'utente.
...
La classica interfaccia di un fornello a gas vede 4 fornelli (in quadrato) e 4 manopole (allineate), ciascuna associata ad un solo fornello.
...
* (qualcuno suggerisce di lasciarle orizzontali e usare un ordinamento più logico, ovviamente non cambia niente)
...
In casi in cui ci sono sale molto grandi con molte luci (come teatri), a volte vengono posizionati gli interruttori direttamente su una pianta del teatro, in corrispondenza delle luci che accendono.
...
* Riassuntiva:
  ...
  * Serve a capire se per avviare gli utenti finali all'uso serve una procedura particolare (una qualche formazione dell'utente)
  * A volte si fà per sapere cosa migliorare alla prossima versione

Approcci:

* Osservazione degli utenti (osservo i comportamenti)
  * In laboratorio
  * Sul campo
  * In remoto
...
* Sul campo
  ...
  * Questa è più realistica, l'utente è più naturale. Comunque Gli utenti possono sentirsi osservati e perdere parte della naturalezza (un tizio di Netflix viene in casa tua e ti guarda guardare serie o ti mette una telecamera), però man mano si abitua
...
Come organizziamo il test? Dobbiamo decidere:
...
* Caratteristiche hardware e software
  * Se sto testando un nuovo dispositivo non abbiamo molto da scegliere
  * Se sto testando un software per una particolare piattaforma devo valutare su che hardware verrà utilizzato (pc nuovo/pc datato/tablet/iPad/...)
...
* Se abbiamo più condizioni da confrontare (versioni diverse dell'interfaccia, interfaccia della concorrenza, etc)
  * Between subjects
    * Ogni utente è assegnato ad una sola condizione analizzata
...
* Analisi statistica
  * Bisogna valutare in che misura i risultati sono dovuti al caso per sapere se la qualità del responso è buona
  ...
  * Non dobbiamo decidere noi dei test statistici speciali, usiamo quelli già forniti da alcuni strumenti come excel, od altri strumenti apposta per l'analisi statistica
    * Esempi:
      ...
      * Se misuriamo quanto tempo ci mettono con due interfacce si fa il T-Test
...
Sono molto utilizzati perché non richiedono di assistere l'utente per la compilazione, quindi è facile farli compilare a molte persone in poco tempo.
...
* Ranking
  ...
  * Si può decidere di permettere pareggi e mettere più item nello stesso livello
    * Come per i differenziali, dipende se vogliamo costrigere gli utenti a schierarsi
* Risposte aperte
...
Adesso è molto più facile svolgere questi sondaggi online inserendo il questionario sul proprio sito, od affidandosi a piattaforme esterne.

Per i questionari online esistono diverse soluzioni gratuite od a pagamento (google forms, surveymonkey, etc.).

I vantaggi dei questionari online sono la vasta quantità e distribuzione degli utenti che rispondono.
...
È difficile controllare la veridicità, spesso si controllano i tempi di compilazione e le risposte contraddittorie, ma soprattutto le seconde.
Si valutano di meno anche i questionari in cui le risposte sono troppo poco distribuite (tutti 0 e 1, tutti a metà, cose del genere).
...
Bisogna costruire un interazione positiva per far capire all'utente che può rispondere la verità.
...
* L'utente assocerà l'azione corretta con l'obbiettivo che vuole raggiungere?
...
Non costringiamo gli esperti a commentare ogni passaggio, li lasciamo liberi di esplorare, ed annotano i problemi quando li trovano.
Serve che usino tutti una terminologia comune, quindi taggano i loro commenti con dei principi dell'HCI (Esempio: violazione di legge gestald, violazione di regola Apple ?...).
...
Metodo GOMS (Goals, Operators, Methods, Selection rules) per predire il tempo che ci mette un utente per svolgere un compito:
...
* Operatori
  ...
  * Saranno azioni del tipo: clicca quella icona, premi questo tasto, etc...
  * Si stima il tempo delle singole operazioni necessarie (esistono delle tabelle)
* Metodi
  * Per raggruppare gli operatori necessari per raggiungere ogni obbiettivo
* Regole di selezione
...
* Portare gli utenti in laboratorio può essere difficile e costoso
...
Un possibile strumento di analisi è Google Analytics, ma ci sono anche molte altre alternative, anche di software libero.
...
Negli anni scorsi i prezzi dei dispositivi di tracciamento oculare sono scesi notevolmente (sono comunque costosi). In ogni momento indicano dove stà guardando l'utente.

Quando guardiamo qualcosa non abbiamo il totale controllo sui movimenti dell'occhio. Quando stiamo fissando qualcosa il nostro occhio fa dei rapidi movimenti strani.
