...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |1.6 tiles|) (read-case-sensitive #t) (teachpacks ((lib "drawings.ss" "installed-teachpacks"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "drawings.ss" "installed-teachpacks")) #f)))
...
(define L-tessellation       ; val: immagine
  (lambda (l)                ; i: potenza di cue (lunghezza lato corto)
    (if (= l 1)
        L-tile
        (let ((s (/ l 2)))
        (glue-tiles
         (L-tessellation s)
         (glue-tiles
          (shift-down (shift-right (L-tessellation s) s) s)
          (glue-tiles
           (shift-down (quarter-turn-left (L-tessellation s)) l)
           (shift-right (quarter-turn-right (L-tessellation s)) l))))))
    ))
