...
public class Huffman {
  ...
  public static void printCodes(String doc, String cod){
    int[] freq = charHistogram( doc );                      // istogramma delle frequenze
    ...
    OutputTextFile out = new OutputTextFile( cod );         // documento compresso
    
    for(int j=0;j<codes.length;j++){
      if(freq[j]>0){
        String ln = j + " ; ";
        switch(j){                                          // riconosce i caratteri speciali
          case '\n':
            ln += "\\n ; ";
            break;
          case '\r':
        ...
        System.out.println((int)((double)j/codes.length*100) + "%");
        out.writeTextLine( ln + freq[j] + " ; " + codes[j] + " ; " + codes[j].length()); // stampa il carattere ed il resto dei dati
      }
    }
  ...
  public static void randomFile(String doc, int length){
    OutputTextFile out = new OutputTextFile( doc );
    
    for(int i=0;i<length;i++){
      out.writeChar((char)(Math.random()*127));
    }
