...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |1.2 puzzle|) (read-case-sensitive #t) (teachpacks ((lib "drawings.ss" "installed-teachpacks"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "drawings.ss" "installed-teachpacks")) #f)))
...
(define croce
  (lambda (l1 l2 s1 s2)
      (glue-tiles
       (glue-tiles
        (glue-tiles
         (shift-right (shift-down (half-turn l1) 1) 2)
         l2
         )
        (shift-right s1 2)
        )
       (shift-right (shift-down (half-turn s2) 5) 2)
       )
      )
  )
...
(define quadrato
  (lambda (l1 l2 s1 s2)
      (glue-tiles
       (glue-tiles
        (glue-tiles
         (shift-right (shift-down l1 1) 2)
         (half-turn l2)
         )
        (shift-right (half-turn s1) 2)
        )
       (shift-right (shift-down s2 5) 2)
       )
      )
  )