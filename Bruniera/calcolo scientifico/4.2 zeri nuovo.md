...
Vogliamo vedere l'errore dell'approssimazione di $\alpha$ ottenuta scegliendo $\tilde\alpha=m=(a+b)/2$ dopo l'ultima operazione.
...
$$
f'(\xi_{a,b})=\frac{f(a)-f(b)}{a-b}~\Leftrightarrow~f(a)-f(b)=f'(\xi_{a,b})({a-b})\\
$$
...
$$
\lim_{k\rightarrow\infin}\frac{|x_{k+1}-\alpha|}{|x_k-\alpha|}=l
$$
...
$$
l=\lim_{k\rightarrow\infin}\frac{|x_{k+1}-\alpha|}{|x_k-\alpha|}=\lim_{k\rightarrow\infin}\frac{|e_{k+1}|}{|e_k|}=\lim_{k\rightarrow\infin}\frac{|g'(\xi_k)e_k|}{|e_k|}=\lim_{k\rightarrow\infin}|g'(\xi_k)|=|g'(\alpha)|\\[2ex]
l=|g'(\alpha)|
$$

#### Ordine di convergenza

Una successione ha ordine di convergenza $p>1,p\in\R$ se:

$$
\lim_{k\rightarrow\infin}\frac{|x_{k+1}-\alpha|}{|x_k-\alpha|^p}=L>0
$$
