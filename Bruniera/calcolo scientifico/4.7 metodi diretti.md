...
Applicando a $b$ le stesse trasformazioni $G$ e $P$, si ottiene direttamente $z=\textcolor{MediumSeaGreen}{(P\cdot G^{-1})}^{-1}\cdot b$.
...
* Ricordando che il determinante di una matrice triangolare è il prodotto degli elementi sulla diagonale, si può usare il metodo di eliminazione di Gauss per calcolare efficientemente il determinante di una matrice:
  ...
  & \Rightarrow\mathrm{det}(A)=\mathrm{det}(\textcolor{LightCoral}U) & [\text{il determinante di $P$ e di $\textcolor{MediumSeaGreen}L$ è 1}]\\
