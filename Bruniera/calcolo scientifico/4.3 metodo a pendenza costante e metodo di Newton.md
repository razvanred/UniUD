...
Con questo metodo, calcolare $x_{k+1}$ equivale a trovare l'intersezione tra una retta di coefficiente angolare $m(x)$ e passante per il punto $(x_k,f(x_k))$, e l'asse delle $x$.
...
Nel metodo a pendenza costante, la funzione $m(x)$ è una costante di valore $m$.
...
$$
g(x)=x-\frac{f(x)}m~\Leftrightarrow~g'(x)=1-\frac{f'(x)}m\\[2ex]
...
$$
g(x)=x-\frac{f(x)}{f'(x)}~\Leftrightarrow~g'(x)=\frac{f(x)f''(x)}{f'(x)^2}\\[2ex]
...
$$
f'(x_k)\approx\frac{f(x_k)-f(x_{k-1})}{x_k-x_{k-1}}\\[2ex]
$$
...
$$
\begin{aligned}
& f(x)=\tilde f(x)-e(x)\\[2ex]
& f(\tilde\alpha)\approx f(\alpha)+f'(\alpha)(\tilde\alpha-\alpha) & [\text{Taylor}]\\[2ex]
& \tilde f(\tilde\alpha)-e(\tilde\alpha)\approx f(\alpha)+f'(\alpha)(\tilde\alpha-\alpha) & [\tilde f(\tilde\alpha)=f(\alpha)=0]\\[2ex]
...
& =|g'(\tilde\xi_k)(\tilde x_k-\alpha)|+\delta_k & [\text{teorema del valore medio tra $\tilde x_k$ e $\alpha$}]\\
...
& \leq\lambda^k(r-\sigma)+\sigma & [\text{$r$ è il raggio di $I_\alpha$},~\text{$\lambda^k$ tende a $0$}]\\[2ex]
