...
public class Board {
	...
	private final SList<Integer> rows,columns,aDiag,dDiag;
	...
	public Board(int dim){
		...
		aDiag  = new SList<Integer>();
		dDiag  = new SList<Integer>();
		textConf = "";
	}
	
	public Board(int dim,int nQueens,SList<Integer> rows,SList<Integer> columns,SList<Integer> aDiag,SList<Integer> dDiag,String textConf) {
		this.dim = dim;
	...
	public Board addQueen(int i,int j) {
		...
		int I = i + 96;//converto l'intero che rappresenta la riga (1,2,..,5) in una lettera ascii (a,b,...,e)
		return new Board(this.dim,this.nQueens + 1,rows.cons(i),columns.cons(j),aDiag.cons(i - j),dDiag.cons(i + j),this.textConf + "  " + (char) I + j);
	}
