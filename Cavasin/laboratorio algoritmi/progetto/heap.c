 ...
 * @return A heap
 */
Heap heapAlloc(int const size, int const maxSize, Comparator compare, void const *const values) {
	int *const refs = malloc((unsigned)maxSize*sizeof(int));
	// populating refs with indices in values
	for(int i = 0; i < size; ++i) {
		refs[i] = i;
	}
	Heap h = {.refs=refs, .refsLength=maxSize, .heapSize=size, .compare=compare, .values=values};
