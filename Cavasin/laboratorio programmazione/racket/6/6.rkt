...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |6|) (read-case-sensitive #t) (teachpacks ((lib "drawings.ss" "installed-teachpacks"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "drawings.ss" "installed-teachpacks")) #f)))
...
(define L-tessellation
  (lambda (n)
    (if (= n 1)
        L-tile
        (let ((tile (L-tessellation (/ n 2))))
          (glue-tiles
           tile
           (glue-tiles
            (shift-down (shift-right tile (/ n 2)) (/ n 2))
            (glue-tiles
             (shift-right (quarter-turn-right tile) n)
             (shift-down (quarter-turn-left tile) n)
             )
            )
           )
          )
...
;(glue-tiles L-tile (shift-down (shift-right L-tile 1) 1))
