...
> È richiesto almeno MiniZinc versione 2.9.3 a causa della issue [919](https://github.com/MiniZinc/libminizinc/issues/919).
...
Per specifica del linguaggio, I record di parametri e variabili vanno dichiarati separatemente e poi uniti con l'operatore `++` (i record devono avere attributi disgiunti).
* **Funzioni**
...
Le iterazioni su `var set` sono tra le operazioni che più appesantisticono il modello fat.
...
In pratica, il vincolo di `occupancy()` impedisce l'esistenza di scatole vuote inutilizzate, e il numero di scatole utilizzate viene calcolato dalla soluzione ingnorando questo valore.
...
### Modello "indicizzato" (`model-indexed.lp`)

Il modello indicizzato è un'iterazione del modello flat che impone l'associazione di un indice univoco ad ogni bottiglia. Il modello chiaramente peggiora dal punto di vista della complessità, ma volevo esaminare le dimensioni del ground.
