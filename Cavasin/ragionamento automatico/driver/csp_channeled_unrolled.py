    ...
    def __init__(
        ...
        used_boxes_dict = {
            bottle["box"]: Solution.Box(
                bottle["box"],
                box_sizes_dict[
                    (box["width"], box["length"], box["height"], box["min_occupancy"])
                ],
            )
            for bottle in solver_solution.bottles
            if bottle["box"] is not None
            for box in (solver_solution.boxes[bottle["box"] - 1],)
        }
...
class Solver(_csp.Solver):
    def __init__(self):
        self.name = config.csp_solver_label + " (channeled, unrolled)"
        solver = MZN.Solver.lookup(config.csp_solver_label)
        model = MZN.Model("model-channeled-unrolled.mzn")
